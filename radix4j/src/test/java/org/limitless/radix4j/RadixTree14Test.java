...
import static org.limitless.radix4j.Node14.Header;
import static org.limitless.radix4j.Node14.Index;
...
public class RadixTree14Test {
    ...
    public void benchmarkMaxLimits() {
        ...
        final var tree = new RadixTree14(RadixTree14.MAX_BLOCK_COUNT);
        long bytes = 0;
        long elapsed = 0;
        for (int i = 0; i < count; ++i) {
            final String str = "abcdefghijklmnop-" + i;
    ...
    private static String getString(Node14 node) {
        final byte header = node.header();
        final int length = Node14.Header.stringLength(header);
